...
public class ConfigManager implements MC {
    ...
    private static void loadFile(final JsonObject input, final Feature feature) {
        for (Map.Entry<String, JsonElement> entry : input.entrySet()) {
            ...
            if (feature instanceof FriendManager) {
                try {
                    GrassWare.friendManager.addFriend(settingName);
    ...
    public void saveConfig(final String name) {
        ...
        if (!path.exists())
            path.mkdir();
        GrassWare.friendManager.saveFriends();
        for (Feature feature : this.features) {
            try {
    ...
    public void init() {
        this.features.addAll(GrassWare.moduleManager.modules);
        this.features.add(GrassWare.friendManager);
