...
import me.alpha432.oyvey.features.setting.impl.StringSetting;
...
public class StringButton extends Button {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (isListening) {
            OyVey.textManager.renderString(currentString.getString() + typingIcon(), x + 2.3f, y - 1.7f - (float) OyVeyGui.getClickGui().getTextOffset(), getState() ? Color.WHITE : new Color(-5592406));
        } else {
            OyVey.textManager.renderString((setting.getName().equals("Buttons") ? "Buttons " : (setting.getName().equals("Prefix") ? "Prefix  " + ChatFormatting.GRAY : "")) + setting.getValue(), x + 2.3f, y - 1.7f - OyVeyGui.getClickGui().getTextOffset(), getState() ? Color.WHITE : new Color(-5592406));
        }
    }
